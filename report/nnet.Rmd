...
$$
\begin{split}
\delta_j^{l_N} &= \frac{\partial C}{\partial a_j^{l_N}} \odot \sigma^{\prime}(z^{l_N}_j) \\
\delta^l_j &= ((w_{jk}^{l+1})^{\top}\delta_j^{l+1}) \odot \sigma^{\prime}(z_j^l) \\
...
\end{split}
$$
Now, for simplicity we set $\frac{\partial C}{\partial w_{jk}^{l}} \equiv \tilde{w}_{jk}^{l}$. Therefore, we can cast all the components above as
$$
\begin{bmatrix} 
